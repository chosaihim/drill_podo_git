...
TaskMotion::TaskMotion(pRBCORE_SHM_REFERENCE _shm_ref, pRBCORE_SHM_SENSOR _shm_sen, pRBCORE_SHM_COMMAND _shm_com, pROS_SHM _shm_ros, JointControlClass *_joint){
    Shm_ref = _shm_ref;
    Shm_sen = _shm_sen;
    Shm_com = _shm_com;
    Shm_ros = _shm_ros;
...
//        std::cout << "Q_filt_34x1[" << i << "] = " << Q_filt_34x1[i] << std::endl;
...
void TaskMotion::RefreshToCurrentReference(void){
    ...
    wbPosWST->SetRetValue(Shm_sen->ENCODER[MC_GetID(WST)][MC_GetCH(WST)].CurrentReference);
//    wbPosWST->SetRetValue(WST_ang*R2D);
...
void TaskMotion::updateAll(){
    if(WBTYPE==WB_POINT_TO_POINT)
    {
        ...
        for(int i=0; i<3; i++){
            ...
            RHdoneflag[i] = wbPosRH[i]->isDoneTraj();
            LHdoneflag[i] = wbPosLH[i]->isDoneTraj();
        ...
        WSTdoneflag  = wbPosWST->isDoneTraj();

        doubles _relb = wbPosELB[0]->UpdateTrajectory();
        doubles _lelb = wbPosELB[1]->UpdateTrajectory();
        RHdoneflag[3] = wbPosELB[0]->isDoneTraj();
        LHdoneflag[3] = wbPosELB[1]->isDoneTraj();
...
void TaskMotion::WBIK_UB(){
    ...
    //----------------------------------------
     ...
     //SH_EDIT
     pPelX = Q_filt_34x1[idX];
     pPelY = Q_filt_34x1[idY];
     //END SH_EDIT
